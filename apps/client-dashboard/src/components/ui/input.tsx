...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  ...
  return (
    <InputPrimitive
      ...
      data-slot="input"
      className={cn(
        "w-full min-w-0 rounded-[var(--radius)] border border-[color:var(--border)] bg-[color:var(--card)] px-3 py-2 text-sm text-[color:var(--foreground)] placeholder:text-[color:var(--form-placeholder)] transition-colors disabled:cursor-not-allowed disabled:opacity-60",
